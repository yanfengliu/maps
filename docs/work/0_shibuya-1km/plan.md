...
Owner disposition, 2026-09-16: `f4e96f0` lands and is pushed the last way a certificate could carry the wrong city's identity. `sceneTreeDigest` in `tools/visual/verify-output.ts` folded in each served file as its path, its size and its modification time; it now folds in the path and the SHA-256 of the bytes, over all 137 files the `/scene/` and `/network/` mounts serve, 371,228,426 bytes, measured at about 0.4 s per call and so about 0.8 s per three-hour gate run. What that bound could not see is a tile rewritten in place at the same length and the same modification time, and the check that was supposed to catch it could not see it either: the case in `test/visual-instrument.test.ts` rewrites a three-byte file and passed whenever both writes landed in one clock millisecond, which it did in 11 of 12 runs on `main`. So this retires an intermittent gate, not a broken one. The content digest makes the case deterministic, the test is unedited at 31 cases, `npm test` is 53 files and 384 tests, `npm run build`, `npm run typecheck` and `npm run audit` are green, and the build bytes are unchanged because `tools/` is outside the bundle graph.
