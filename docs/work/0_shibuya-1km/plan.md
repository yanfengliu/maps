...
| Phases 4–5 | The isolated two-style candidate retains scoped acceptances for C5, C6, C7b and C9, plus F8 and F10; no graphics code is committed. The renderer-identity cache candidate passed four nonvisual gates and 271 unit tests, but ordinary 15-second reload still failed. Its screenshot-only ablation also failed noon navigation after setup; root accepted the sealed negative evidence, unchanged inputs and completed cleanup. Removing screenshot recording was insufficient in that run, not zero observer cost or a proved product cause. Earlier eight hero and two return views retain their scoped native review; the 36-view sweep and full 44-view graphics gate remain incomplete. | Renderer worker implements and verifies; root orchestrator accepts the integrated result after independent review. |
...
| Phases 7–8 | Population simulation is not integrated. The finite one-vehicle app retains scoped CPU acceptance. The later ordinary 18-execution run was canceled after eight completed cases and 56 frames because buildings hid required moving views; a ninth case produced no frames and nine did not start. Response records passed for the eight completed cases. Review 17 accepts only the CPU visibility input-contract repair; its live adapter and full 18-execution/126-image native verification remain pending. Review 16 accepts the later gait selector on all 43 cases at 240 Hz and sampled 60 Hz, with earlier failures retained. Production gait, between-tick poses, naturalness, terrain/LOD contact and populated motion remain unintegrated. | Network worker owns simulation mechanics; asset worker owns source provenance, geometry, animation and rendering; root orchestrator accepts shared pose and fleet contracts. |
| Phases 9–10 | The one-rig, twelve-stream CPU diagnostic measured a cost problem: source/IK poses for 3,000 agents take 96.988 ms median; same-tick update plus pose takes 101.412 ms median. This is not heterogeneous population or whole-scene performance. Final integrated simulation, rendered quality and the full acceptance gates remain pending. | Root orchestrator owns measured performance, whole-deliverable acceptance and merge to main. |
