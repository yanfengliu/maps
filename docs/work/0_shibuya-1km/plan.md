...
Independent review of the corrected two-lane instrument reports no blocking finding (`artifacts/instrument-review/review.md`, SHA-256 `0fb106584c8dff4d306940673672be7f730a5c2dfbc25456f176d4bb47ac699f`). Its five accepted material findings were described here as "taken into the hardening unit that follows", and that is corrected on 2026-09-16: on `main` at `04be817` **none of the five is fixed**, and each is named below with where it lives now. (1) The previous `complete.json` survives a failed build, because `package.json`'s `visual` chain runs `npm run build` before `node tools/visual/verify-output.ts --begin` and the deletion sits inside `beginVisualRun` at `tools/visual/verify-output.ts:21`. (2) `complete.json` carries no lifecycle evidence: `verify-output.ts:50` writes `run.json`'s keys (`startedAt`, `lane`, `requestedGpu`, `build`) plus `completedAt` and `frames`, and no step of the wrapper reads `artifacts/visual/lifecycle/`. (3) The build-hash binding excludes `data/scene`: `verify-output.ts:22` hashes `dist/index.html` and `dist/assets/*.js|css` only, and `data/scene` appears in neither `run.json` nor `complete.json`. (4) The lifecycle lane has no console or pageerror assertion: `tools/visual/lifecycle.spec.ts` attaches no `page.on("console")` and no `pageerror` handler, where `tools/visual/hero.spec.ts:66-70,312` and `tools/visual/sweep.spec.ts:68-72,240` both attach and assert one. (5) The pixel lane's SwiftShader identity is recorded and never asserted: `tools/visual/sweep.spec.ts:320` and `tools/visual/hero.spec.ts:215` write `glRenderer` into the manifests, `playwright.config.ts:64-66` pins `--use-angle=swiftshader`, and `verify-output.ts` never reads the string it recorded. The fix for all five exists only as uncommitted work in the `artifacts/gate-hardening` worktree on branch `worker/gate-hardening`, whose base `e561d75` is 22 commits behind `main` and which has no commits of its own (`git log main..worker/gate-hardening` is empty). Its `REPORT.md`, SHA-256 `03D88804406372843E6BECF767F3B66EB99F233990B04A675B56FB99671968EF`, records that nothing was committed, pushed or merged, so a reader must not cite that worktree as a fix on `main`. One claim is rejected with evidence: the review states that the wrapper's deadline "does not exist anywhere in the tree", and the policy does state it.
