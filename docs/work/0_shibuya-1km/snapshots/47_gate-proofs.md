...
Two mutations were observed red on base `27c71865bef31a7c9dcc622db16e6721be839dc3`: removing the preflight recreates the old destructive orchestration, and moving cleanup before preflight erases the markers before inspecting them. Each produces 23 failures and three passes, with `promise resolved "undefined" instead of rejecting`. The finally-restoring runner verifies the restored helper SHA-256 `6b01b2bb5ea70c8357c377e403eb10837b6b9611bf49eba64e3a166c2f97430a`. Restored focused tests pass 27 cases across preservation and cleanup. Evidence remains under `artifacts/rebuild-preservation/wt/artifacts/rebuild-evidence/`: `order-control.mjs`, `order-control.json`, both mutation stdout/stderr pairs and `restored-green.stdout.log`.

Bound: temporary roots with the current producer format and finite corruptions. No actual source fetch, shared scene writer, full city rebuild, GPU or post-admission rollback was exercised. The preflight recognizes the existing output format; it does not authenticate original source bytes or authorize an authored regeneration path. No queued lesson is retired by this gate.

## 2026-09-19 — emitted flicker capture validity and unavoidable observer settlement

Gate: `test/flicker-capture.test.ts` exercises the real emitted manifest through `capture.spec.ts`, with external browser/input/disk I/O replaced. The real decoder, copy validators and judge process twelve synthetic 192x108 frames. `test/flicker-burst.test.ts` separately exercises actual `MotionPath.during` and a deferred observer.

Control: temporarily restore `judge.ts`, `capture.spec.ts` and `motion.ts` from the exact rejected Q3 freeze (`candidate.patch` SHA-256 `9e65da1a42c5a3f87cdbddbd0f8114513e4042e75221887b40f89ced1aa1b9d4`). Five assertions fail: a stale sequence reports valid; counter and missing-copy refusals have empty capture reasons; both combined input/release failure cases return before the observer settles. The script verifies original source hashes and restores candidate bytes in finally. Evidence: `artifacts/motion-repair/wt/artifacts/motion-repair-evidence/old-red-final.log` and `old-red-final.json`. The earlier malformed test-fixture PNG encoding is retained separately and is not semantic evidence.

Restored result: 54 focused tests pass across seven flicker files, and typecheck passes. A CPU replay verifies all 24 original probe3 PNG hashes and both manifest hashes, then runs their exact bytes/receipts through the repaired specification. Both captures are valid with zero capture refusals; every pair measurement is unchanged and all 22 residual failures remain. No threshold or estimator changed.

Bound: capture/refusal classification and cleanup for finite input/observer failures, not arbitrary never-settling promises or scene stability. The existing eight-second observer timer bounds a responsive page; Playwright's seven-minute test deadline and the owned process runner remain the unresponsive-page backstop. Review 43, primary integration and the five full gates remain owed. No queued lesson is retired here.
...
**Bound.** The fixture models the drag's damping as a constant fraction of the commanded angle, 0.52, measured once on 2026-09-17; the browser's own delivery is asserted only by `npm run visual:flythrough`, which this unit does not run. The ground profile is the served terrain's shape over the crowd's knot, not the mesh itself, so the fixture proves the loop converges under that model and the judge names a stand left under the floor — it does not prove what the controls deliver, and it opens no frame. The 0.002 rad stopping rule inside `standConverge` is not exercised by any case, and it is **unreachable as the code stands**: the height error a 0.002 rad angle corresponds to is at most `distance * 0.002`, and the controls' own distance floor is 25 m, so any angle under that bar is a height already inside the 0.05 m convergence bar and the helper returns "already within" first. The case says so in as many words rather than asserting a branch it never reaches.
## The flythrough's aim scorer refuses a camera whose ground makes its axis unusable

**Gate:** `npm test` � `test/flythrough-aim.test.ts`, two cases over the pure scorer in `tools/flythrough/aim-score.ts`. The first drives a synthetic two-plateau dump through `scoreDump`/`bestPerCell` and holds the old rule, written out inline as `oldScorer`, as its own control; the second drives a valley camera whose stand height is below the app's pinned target.

**Landed:** 2026-09-17 in the worktree `artifacts/crowd-aim-fix/wt` on branch `crowd-aim-fix` off `368f92c`. Not merged: the coordinator lands the branch. Fixes item 2.2 of `artifacts/quality-audit/register.md`.

**Why.** The tool that chose the flythrough's crowd anchor scored every visible pedestrian at `TARGET_Y_M + bodyHeightM` � a constant 16.1 m in world Y, whatever the terrain under the body said � and required only that the *target's* ground be non-NaN. At the anchor it picked, `(-386.198, -496.297)`, the camera stands on 26.36 m of ground and the app pins the controls' target at 15.2 m, so the real optical axis is **34.70 degrees down**; the flight carried that pose and `frames/crowd/crowd-000.png` is 311 pedestrians projected onto rows 636-710 of 720, which is a photograph of paving. The scorer now scores each body at `ground(body) + 0.9`, projects it into the renderer's own camera frame (`lookAt` construction, world up), and refuses a pose whose axis is steeper than 20 degrees or which holds fewer than 10 walking bodies inside 45 m.

**Mutation � the steep-axis refusal disabled:** in `tools/flythrough/aim-score.ts`, `if (pitchDegrees > options.maxPitchDegrees) {` ? `if (false) {`, nothing else changed. `npx vitest run test/flythrough-aim.test.ts`, exit status 1, in 0.4 s:

```
FAIL test/flythrough-aim.test.ts > the flythrough's aim scorer > does not rank the high-ground camera first, where the old scorer would have
AssertionError: the refusal has to name the steep axis rather than a missing crowd: this case is about the axis, and a fixture whose refusal came from the near-range rule would pass this file while the steep-axis rule was deleted: expected '0 walking bodies are inside the frame within 45 m of the camera and this leg needs 10: a crowd further away than that is a count of figures too small to judge. The nearest body in frame is none' to match /degrees down/
```

**Green on the restored bytes:** 2 passed in 0.5 s.

**What the fixture is, and its control.** Two plateaus 13.2 m apart, as an explicit `plateauGround` function rather than `groundGrid` � a grid answers with the highest vertex in a cell, and where the cells fall decides whether a query on the low side reads the low plateau or the cliff above it, which is exactly the fragility a 13.2 m cliff introduces. The high camera stands on 27.5 m of ground with a 21x21 knot 30 to 70 m away; the low camera on 14.3 m of ground with a 5x5 knot 8 to 12 m away. `oldScorer` is the rule at `368f92c`: the constant body height, the ray drawn to that same constant, no near range, and no standability question. It has to have counted a crowd at the high-ground camera for the case to mean anything, and the file asserts that before it asserts the disagreement. The high camera's poses are then all refused, absent from the ranking rather than beaten, and the ranking opens on the low camera.
...
**Failure, on the merged anchor:**

```
5 of 6 held frame pairs show a scene that went still while the camera was deliberately held
(frames/crowd/crowd-007.png: the same bytes as the previous frame; ...). The hold is the lane's control meas...
...
**Why this is a gate and not a preference.** The measured defect is that a surface renders below the frame's own black threshold at the darkest preset. The check cannot assert the pixel, so it asserts the input the pixel is made of, with the conversion between them recorded in the case: both fits (`radiance = A + K�albedo`, and the single-point fit that assumes no albedo-free light) are calibrated on the certified frames' own measurements and must reproduce them before they are used, and the case fails if the albedo leaves the band real asphalt occupies.
...
## The flythrough's crowd anchor names the scored pose's two ends, and the flight's camera is the other end of that ray

**Gate:** `npm test` - `test/flythrough-crowd-anchor.test.ts`, the case "names the scored pose's two ends, and the flown camera is the other end of that ray".

**Landed:** 2026-09-18, branch `fix/flythrough-anchor-constant` off `dbb7e3d`. Not merged: the coordinator lands it.

**The defect it gates.** `tools/flythrough/aim-score.ts` builds each candidate's target as `camera + (sin az, cos az) * distance`, so its `azimuth` is the direction the camera **looks**. `CameraSnapshot.azimuth` is `controls.getAzimuthalAngle()`, which measures from the target **to** the camera. `plan.ts` hands the scorer's number to the controls unchanged, so the exported `CROWD_CAMERA` (the scored camera, `(456.0, 389.7)`) and the camera the flight actually flies (`(462.79, 441.26)`) are the two ends of one 26 m ray, **53.1 m apart**, while the comments in the plan, the aim tool and the scorer all called the same angle by the controls' name. The `CROWD_CAMERA` docstring claimed "the camera stands at `target + (sin, cos) * distance`" and then derived the target from the camera with that formula, which is the 180 degree confusion stated in prose and contradicted by the code beside it.

**Mutation, the role swap.** In `tools/flythrough/plan.ts`, the target's derivation `CROWD_AIM_CAMERA.{x,z} + Math.sin/cos(CROWD_AZIMUTH) * CROWD_DISTANCE_M` changed to `-`, nothing else. This is the edit the false comment invites: it makes the exported pair's own target-to-camera bearing equal `CROWD_AZIMUTH` instead of that plus a half turn. `npx vitest run test/flythrough-crowd-anchor.test.ts`, exit status 1, in 0.4 s:

```
AssertionError: CROWD_AIM_CAMERA is at a target-to-camera bearing of 7.500 degrees from CROWD_TARGET where
CROWD_AZIMUTH plus 180 is 187.500. The scorer builds its target as `camera + (sin az, cos az) * distance`, so the
exported camera sits 26 m *behind* the target: the target is ahead of it at exactly `CROWD_AZIMUTH`, and the
pair's own bearing is that plus a half turn: expected 3.141592653589793 to be less than 1e-9
```

**What the case pins, from the run and not from a comment.** Every number in it is read from `artifacts/flythrough2/manifest.json`, the passing run: the opening pose's camera `(216.341, 554.452, 216.341)` against target `(0, 15.2, 0)` at `azimuth` 0.7853982, `polar` 0.5160775 and 620 m (the controls' formula including its `sin(polar)` term, matched to under a centimetre); the crowd leg's opening frame, camera `(462.439, 17.232, 442.423)` against target `(459.056, 15.200, 416.724)` at `azimuth` 0.1308997 and `distance` 26.000001; and the two legs' own pan targets. It then asserts the exported pair's mutual bearing is `CROWD_AZIMUTH + PI`, that `CROWD_FLOWN_CAMERA` is 26 m from the target at bearing `CROWD_AZIMUTH`, that the two cameras are 52 m apart, and that the derived camera is within the approach's recorded pan residual (0.909 m) of where the run opened.
...
**The bar, and the control on each side of it.** `CRAWL_FRACTION_PER_FRAME` is `0.005` of the frame's pixels per rendered frame, and a pair's bound is that times its own measured frame gap. The negative control - ten frames of the synthetic field, each a pure 3 px, 1 px translation of the one before - measures **5.934e-5 to 1.563e-3** per frame. The positive control - the same movement with the detail collapsed on alternate frames - measures **5.154e-2 to 5.326e-2**. (Both bands corrected 2026-09-18 by round 35, which re-ran the shipping judge over the shipped fixtures; the 1.2e-4 to 1.6e-3 and 3.7e-2 to 5.2e-2 this entry carried are remembered numbers the fixture does not produce. The separation is wider than claimed, so no verdict moves - and the test never asserted the bands, only that each control falls on its own side of the bar.) The first case asserts the pure shift stays under a third of the bar and the crawl case asserts it is above it, and the crawl case asserts the separation is more than tenfold, so the two controls pin the bar rather than each being checked alone. Every frame pair's crawl message carries its own arithmetic; the crawl case's is, verbatim from the run:
...
**M2 - `channelDelta` called with pixel indices instead of byte offsets** (`at * 4, source * 4` → `at, source`). Exit 1, 2 of 13 red. **This is the mutation of the defect that was live in `main`.** **Corrected 2026-09-18 by round 35: this sentence's provenance was inverted.** The *second* message is the 192x108 fixture's own case (`passes a moving camera whose frames differ only by the expected shift`), and the *first* is the 1280x720 case (`counts changed pixels at the size the lane actually captures`) - so neither is a message the other's fixture could never produce, and no fixture was too small to reach the defect. What hid it was the second case's pre-fix assertion, `expect(pair.changedFraction, ...).toBeGreaterThan(0.5)`, written to the bug's own output:
...
**M4 - `deriveStep` subtracting the epoch origin again** (the RUN-01 arithmetic, `timeOrigin = 1_789_702_869_047.3`). Exit 1, 5 of 5 red. **Corrected 2026-09-18 by round 35: the first message does not reproduce RUN-01's recorded tick.** RUN-01 recorded `-107382152727` (`artifacts/flicker/RUN-01-REPORT.md:96`); the mutation reads `-107382172011`. The other two messages reproduce exactly, so the watched-red proof stands and the provenance sentence does not:
...
- The byte-offset case gates the arithmetic at the size the lane captures (1280x720). **Its bound is the frame size and not an out-of-bounds read, and round 35 corrected that:** RGBA pixel indices are in bounds at every image size (921,599 against 3,686,400 bytes at 1280x720; 20,735 against 82,944 at 192x108), the mutation returns a finite `changedFraction` of 0.9702, and the defect was hidden by an assertion calibrated to its own output rather than by a fixture that could not reach it. What this gate covers is this conversion at this size; the class it belongs to - a unit conversion whose wrongness is a matter of degree rather than of an exception - is caught only by exercising the real dimensions, which this file's other unit fixtures do not do.
...
- The cadence case gates a constant. It cannot see what a pair actually spanned, which is the run report's measured `frameGap`, and it deliberately spans 5 frames in the fixture rather than the ~21 the lane achieves. The coupling is worse than "four times looser": `allowed` is 0.005 times the span (0.105 at 21 frames against 0.025 at the fixture's), but `crawl` is the unexplained fraction already divided by the span, so the predicate is `unexplainedFraction > 0.005 * frameGap^2` - it tolerates 2.205 of the frame at 21 frames and cannot fire at all past `sqrt(1/0.005) = 14.14`. The fixture's span is what makes the positive control fire; at a 12-frame span it reads 0.022 and falls *below* its own 0.088 bound. That coupling is a defect in the bar, carried in the run report, and it is why this run's `no failures` does not mean the crawl class was enforced.
...
**Measured after the fix**, from the lane's own two runs on the final scene (frames bound by SHA-256 in `artifacts/flicker/capture/manifest-*.json`): the picture moves 1-2 px per pair by the estimator's own answer, 0.1911-0.2016 m and 0.0042-0.0045 rad of camera motion, and `changedFraction` reads 0.1871-0.2206 where the defect made it 0.97. The worst real pair reads `crawl` **1.351e-3 per frame**; **that is a measurement at the criterion's regime and not a gate passed against `0.005`, which the judge never applies to a pair.** A pair's allowance is `0.005 * frameGap` (`judge.ts:504`) and the predicate behind it is quadratic in the span, so at these pairs' 20.5-23 rendered frames the class cannot fire at all (see `docs/policies/local-rules.md`). The bar is still provisional and this file does not claim it is calibrated: see `artifacts/flicker/RUN-02-REPORT.md` for why the available controls cannot calibrate it.
...
## 2026-09-19 — Adjacent capture and wheel input controls

On base `27c71865bef31a7c9dcc622db16e6721be839dc3`, the new seven-case `flicker-adjacency.test.ts` produced five failures before the judge changed: detail-collapse detection at gaps 15/22/100, rejection of a 22-frame pure-translation gap, and rejection of a multi-frame shutter. Restoring direct unexplained-fraction comparison and exact frame binding passes all seven. Evidence: `artifacts/motion-capture/artifacts/motion-evidence/old-red.log`.

Removing `mouse.up()` immediately before the ascent wheel dispatch makes `flicker-burst.test.ts` fail `OrbitControls ignores wheels while dragging: expected true to be false`. The candidate is restored in finally. Evidence: `artifacts/motion-capture/artifacts/motion-evidence/wheel-drag-red.log`; hardware probe2 records the ignored wheels and probe3 the measured in-burst distance increase. Blank, stale, nonadjacent and unrun controls remain in the focused suite. No lesson is retired: the integer-only classifier is still not a scene acceptance gate. [Bounds and native frames](../work/0_shibuya-1km/snapshots/q3-motion-capture-review43.md).

## Pedestrian route completion needs physical terminal clearance, 2026-09-19

Gate: test/pedestrian-terminal.test.ts. Bound: synthetic late-primitive compound, three actual source radii/tails, one-walker fixed-step lifecycle through two generations, and actual RenderLoop continuation under a deliberately unsafe plan. The final related gate is 24 passing cases across four files. This does not establish 3000-body throughput, collision-free walking, visual continuity or pedestrian AOI egress.

Three restored mutations were watched red. Replacing clearPedestrianTerminal with its unchanged prefix fails the real tick's completion assertion with expected 0 to be greater than or equal to 2. Bypassing occupied retirement protection fails expected +0 to be 1 for the active body. Reusing the cached pre-integration footprint fails exact current-position correspondence: expected Z -2.2461535930633545, received -2.2469472885131836. Raw logs and the finally-restoring runner are artifacts/lease-retirement/artifacts/lease-evidence/red-controls.ps1 and red-{missing-tail,occupied-retirement,stale-footprint}.log. The separate prototype report preserves the original observation. No existing lesson is retired by this change.
