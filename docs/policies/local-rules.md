...
The owner's 2026-09-15 team instruction is that the coordinator runs a team and sizes it to the work, rather than doing the work itself. The coordinator's own hands stay on reading enough to write an assignment and on inspecting a handoff to accept or reject it. Everything else is delegated, including the small changes, because the coordinator's session is where the next request arrives and a coordinator in the middle of an edit cannot take it.

Sizing the team is a running decision, not a one-off. Widen it when the critical path is a single lane and independent work is waiting; narrow it when lanes duplicate, contend for the same file, the same port or the same CPU, or when a lane has nothing to do until something else lands. Every lane has an owner, one bounded outcome and a written place to hand off; a lane with none of those is not a lane. Record the roster and what each lane is waiting on in one place that outlives the conversation, and update it when a lane ends. Three things make a lane unsafe to run: it edits a file another lane is editing, it runs a gate that another lane is running, or it needs a resource the machine does not have. This repository makes the third concrete rather than hypothetical — `npm run visual` is a three-hour command that owns port 4319 and the whole CPU, so exactly one gate run happens at a time and lanes are chosen around it.

The gate is the reason delegation is not optional here. A coordinator that implements spends the same three hours on one change that a team of lanes would spend on four, and the fleet constitution already forbids it. What the coordinator owes each worker is in the assignment: owner, outcome, base revision, workspace, allowed and excluded changes, the contract it must honour, how it will be verified, and what its handoff must contain. What the coordinator owes the repository is the roster, the decisions that cross lanes, and the acceptance of the integrated result rather than of a worker's summary of it.
