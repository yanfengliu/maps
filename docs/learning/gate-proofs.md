...
**Measured after the fix**, from the lane's own two runs (frames bound by SHA-256 in `artifacts/flicker/capture/manifest-*.json`): the picture moves 1-2 px per pair by the estimator's own answer, 0.1963-0.1964 m and 0.0044 rad of camera motion, and `changedFraction` reads 0.187-0.207 where the defect made it 0.97. The worst real pair reads `crawl` **1.291e-3 per frame** against the shipped `0.005`. The bar is still provisional and this file does not claim it is calibrated: see `artifacts/flicker/RUN-02-REPORT.md` for why the available controls cannot calibrate it.
