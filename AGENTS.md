...
| Visual | `npm run visual` | Claims the run before the build, builds once, then two lanes: the 44-frame software pixel sweep (SwiftShader) and the lifecycle check on the hardware renderer, repeated three times, before the wrapper certifies the complete frame set against both lanes' renderers, the three lifecycle records and the served scene data. The software lane can take several hours; measured finite budgets are in the local rules. |
...
`npm run visual` boots the production build and drives OrbitControls with synthesised pointer and wheel input. It writes 44 frames under `artifacts/visual/`: eighteen sweep views per style (six azimuths at three distances) plus eight hero views (both styles, dusk/noon and crossing/approach). Per-specification manifests record camera poses and SHA-256; the final wrapper checks every expected frame, fresh timestamps, unchanged build bytes and the scene data the preview server served, requires the three hardware lifecycle records for this run, and asserts that both lanes reported the renderer they are split on, before writing `complete.json`. Its first step deletes the previous certificate, ahead of the build, so a failed run cannot leave one; a run with no step flag is refused rather than re-certifying what is on disk. It waits for tiles to stop refining before every capture. The complete 44-frame wrapper is implemented; final integrated capture and native inspection remain pending in the active plan.
...
Five of the instrument's own checks have been made to go red, with the mutation and the message recorded in [docs/learning/gate-proofs.md](docs/learning/gate-proofs.md): a certificate surviving a failed build, a certificate issued without the hardware lifecycle lane, one issued for frames captured against a scene that changed mid-run, a lifecycle lane whose cleanup threw, and a pixel lane whose manifests name another renderer. Six more are recorded there as the gate's earlier proofs. Three failure paths are written and reachable but have never been watched to fire — WebGL unavailable, the render loop stopping mid-sweep, and the camera never settling. Treat those three as code, not as evidence.
